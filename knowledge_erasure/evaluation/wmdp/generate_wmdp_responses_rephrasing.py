...
def main(args):
    ...
    if args.dev_task == "":
        output_breakpoint_name = f"{ROOT_DIR}/run_breakpoint_{args.extra_info}_rephrasing.json"
        output_filename = f"{ROOT_DIR}/run_results_{args.extra_info}_rephrasing.json"
    else:
        output_breakpoint_name = f"{ROOT_DIR}/run_breakpoint_{args.extra_info}_{args.dev_task}dev_rephrasing.json"
        output_filename = f"{ROOT_DIR}/run_results_{args.extra_info}_{args.dev_task}dev_rephrasing.json"
...
def generate_results_for_prompt(
    ...
    def load_json(task):
        res = []
        with open(os.path.join(args.data_dir, directory, "test", task + '.json'), "r") as f:
            for line in f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--system_prompt", type=str,
                        ...
                        # default="The following are multiple choice questions (with answers). Please answer the question in A, B, C, or D.\n\n")
