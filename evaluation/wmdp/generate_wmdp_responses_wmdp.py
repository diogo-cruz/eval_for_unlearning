...
def generate_results_for_prompt(
    ...
    def load_df():
        import ast
        with open(Path(args.data_dir) / "bio_questions.json") as f:
            tmp = f.readlines()
            tmp = [ast.literal_eval(x) for x in tmp]
            tmp = pd.DataFrame.from_records(tmp)
            tmp = pd.concat([
                tmp["question"],
                tmp["choices"].apply(pd.Series),
                tmp["answer"].apply(lambda x: chr(65 + x)),
            ], axis=1)
    ...
    for task in TASKS:
        ...
        for i, question in enumerate(test_data):
            ...
            # dev_df_valid = dev_df[dev_df[0].str[:100] != question["question"][:100]]#.sample(len(dev_df)-1, random_state=42, replace=False)
            # assert len(dev_df) == len(dev_df_valid) + 1
            dev_df_valid = dev_df.drop(i)

            # train_prompt = gen_prompt(
            #     dev_df_valid, k
            # ) if args.dev_task != "" else ""

            train_prompt = gen_prompt(dev_df_valid, k)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--system_prompt", type=str,
                        ...
                        # default="The following are multiple choice questions (with answers). Please return your answer in the form of A, B, C, or D.\n\n")
