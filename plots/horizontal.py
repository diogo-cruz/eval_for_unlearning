...
gs = fig.add_gridspec(2, 2, height_ratios=[1, 4], width_ratios=[1,.95])
# gs = fig.add_gridspec(2, 3, height_ratios=[1, 4], width_ratios=[1.01,.001,.9])
...
plt.subplots_adjust(hspace=0.11)  # tighten vertical spacing
plt.subplots_adjust(wspace=0.22)  # tighten vertical spacing
    ...
    for _, row in merged.iterrows():
        ...
        if task == 'wmdp_bio_rephrased_hindi_filler':
            if family['name'] == 'Zephyr':
                ...
                main_ax.text(
                row['adjusted_accuracy_base'] - .007,
                row['adjusted_accuracy_elm'] - 0.027,
                'Hindi filler',
                fontsize=11,
                alpha=0.85
            )
...
main_ax.set_ylabel(r'Unlearned (ELM) Model Accuracy (Adjusted)', labelpad=label_pad)
...
ax.set_xlabel('Base Model (Llama3-8B-Instruct)\nAccuracy (Adjusted)')
ax.set_ylabel('Unlearned Model Accuracy (Adjusted)', labelpad=label_pad)
...
ax.plot([-.01, 1], [-.01, 1], 'k--', alpha=0.3, zorder=6)
